dnegmix <- function(x, par, family = c("normal", "gamma")) {
  # --- Family check
  ...
  if (family == "normal") {
    return(as.numeric(dnormix(x, par$w_p, par$mean_p, par$sd_p) - dnormix(x, par$w_n, par$mean_n, par$sd_n)))
  } else if (family == "gamma") {
    return(as.numeric(dgammix(x, par$w_p, par$shape_p, par$rate_p) - dgammix(x, par$w_n, par$shape_n, par$rate_n)))
  }
}